...
document.addEventListener("DOMContentLoaded", () => { //
  ...
  /**
   * Substitui addFeed. Adiciona a mensagem ao feed apenas se for diferente da última.
   * Usar safeFeed(type, msg) em TODOS os lugares que chamavam addFeed.
   */
  ...
  // ===================== NOVOS HELPERS BLOQUEIO (REFAZENDO) =======================
  ...
  function onNewCandle(arr){ //
    ...
    // ===================== ATUALIZAÇÃO DO NOVO BLOQUEIO (G1/G2) =====================
    ...
    // OBS: O bloqueio é anulado pela força, mas a variável de estado só é resetada aqui.
    ...
    if(pending && pending.enterAtIdx === last.idx){ //
      ...
      if(win){ //
        ...
        safeFeed("ok", `WIN 2x (G${pending.stage})`); // Substituído addFeed por safeFeed
        ...
        if(pending.stage === 0){ //
          safeFeed("err", "LOSS 2x (G0)"); topSlide("LOSS 2x", false); // Substituído addFeed por safeFeed
          ...
          safeFeed("warn", "Aguardando novo gatilho para G1"); // Substituído addFeed por safeFeed
          // Continuar
        }
        if(pending.stage === 1){ //
          safeFeed("err", "LOSS 2x (G1)"); topSlide("LOSS 2x (G1)", false); // Substituído addFeed por safeFeed
          ...
          safeFeed("warn", "Aguardando novo gatilho para G2"); // Substituído addFeed por safeFeed
          // Continuar
        }
        if(pending.stage === 2){ //
          safeFeed("err", "LOSS 2x (G2)"); topSlide("LOSS 2x (G2)", false); // Substituído addFeed por safeFeed
          stats.losses++; //
          stats.streak = 0; //
          syncStatsUI(); store.set(stats); //
    ...
    if (isSeguidinha && !window.seguidinhaOn) { //
      ...
      safeFeed("info", `SEGUIDINHA ON: ${lastN.length} velas, ${blueInLastN} azul`); // Substituído addFeed por safeFeed
    }
    if (!isSeguidinha && window.seguidinhaOn && lastN.length >= 6) { //
      window.seguidinhaOn = false; //
      safeFeed("info", "SEGUIDINHA OFF: padrão quebrado"); // Substituído addFeed por safeFeed
    }
    ...
    if(pending?.stage === 'G1_WAIT' || pending?.stage === 'G2_WAIT'){
      ...
      // 3. BLOQUEIO XADREZ (REGRA IGUAL ANTES: BPBP/PBPB → só libera depois que confirmar 1 blue)
      // O BLOQUEIO XADREZ não é anulado pela força, mas a força é a prioridade máxima para entrada.
      ...
      pending.strategy = analysis?.name || (forceDetected ? "forca" : "seguidinha"); // Se não houver estratégia forte, usa "forca" ou "seguidinha"
    ...
    if (currentBlueRun >= 3) { //
      if (waitingForNewCorrections === 0) { //
        ...
        safeFeed("warn", "Bloqueio: 3+ azuis. Aguardando 1 nova correção ≤2."); // Substituído addFeed por safeFeed
    ...
    if (waitingForNewCorrections > 0) { //
      ...
      if (lastChange && lastChange.streak <= 2) { //
        ...
        safeFeed("info", "Desbloqueado: nova correção ≤2 confirmada."); // Substituído addFeed por safeFeed
    ...
    if (!pending && waitingForNewCorrections === 0) { //
      if (window.seguidinhaOn) { //
        ...
        safeFeed("warn", `SINAL 2x (G0) — SEGUIDINHA ON`); // Substituído addFeed por safeFeed
        return; //
      }
      ...
      if (allowEntry) { //
        ...
        safeFeed("warn", `SINAL 2x (G0) — entrar após (${pending.afterMult})`); // Substituído addFeed por safeFeed
      } else {
    ...
    try{ //
      ...
      setInterval(() => { //
        if (waitingForNewCorrections > 0 || !pending && engineStatus.textContent.includes("bloqueado")) { //
          ...
          window.prevCorrGate = null; //
          safeFeed("info", "Reset automático após inatividade."); // Substituído addFeed por safeFeed
          engineStatus.textContent = "operando"; //
        }
      }, 1800000); // 30 min //
    }catch(e){ //
