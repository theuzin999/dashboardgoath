...
function setCardState({ active = false, awaiting = false, title = "Chance de 2x", sub = "identificando padrão" }) {
    ...
    chanceCard.classList.remove("chance-active", "chance-awaiting");
    if (active) {
        chanceCard.classList.add("chance-active");
        flashCard();
...
function renderHistory(list) {
    ...
    last15.forEach(r => {
        ...
        val.className = "val";
        val.textContent = r.mult.toFixed(2) + "x";
        const dot = document.createElement("div");
        dot.className = r.color === "blue" ? "dot-blue" : (r.color === "purple" ? "dot-purple" : "dot-pink");
        const c = document.createElement("div");
        c.className = "c";
        c.textContent = r.color;
        top.appendChild(val);
        top.appendChild(dot);
        box.appendChild(top);
        box.appendChild(c);
...
const store = {
    get() {
        try {
            return JSON.parse(localStorage.getItem("stats2x") || "{}");
        } catch {
            return {};
        }
    },
    set(d) {
        localStorage.setItem("stats2x", JSON.stringify(d));
    }
};
...
function syncStatsUI() {
    ...
    normalWinsEl.textContent = stats.normalWins;
    g1WinsEl.textContent = stats.g1Wins;
    g2WinsEl.textContent = stats.g2Wins;
}
syncStatsUI();
...
// Utils
function colorFrom(mult) {
    if (mult < 2.0) return "blue";
    if (mult < 10.0) return "purple";
    return "pink";
}
...
function detectStrategies(colors) {
    ...
    if (b === "purple" && c === "purple" && window.hasRecentPurpleRun3) return { name: "sequência roxas", gate: "run≥3 recente + PP ⇒ P" };
            ...
            if (win) {
                ...
                if (pending.stage === 0) {
                    if (awaitingStability) {
                        ...
                        setCardState({ active: false, awaiting: true, title: "aguardando estabilidade", sub: "G1 parado" });
                        ...
                        addFeed("warn", "Ativando G1");
                    }
                } else if (pending.stage === 1) {
                    pending.stage = 'G2_WAIT';
                    pending.enterAtIdx = null;
                    martingaleTag.style.display = "inline-block";
                    setCardState({ active: false, awaiting: true, title: "aguardando estabilidade", sub: "G2 parado" });
