<!-- PROTECT_SNIPPET: cola no <head> de cada HTML protegido -->
  (function protectPage() {
    const API_BASE = "https://keysdash.espanhaserrita.workers.dev/"; // <--- ALTERAR se necessário
    ...
    (function antiDevTools() {
      ...
      document.addEventListener("keydown", e => {
        if (e.key === "F12" || (e.ctrlKey && e.shiftKey && ["I","i","J","j","C","c"].includes(e.key))) {
          e.preventDefault();
          window.location.replace("/"); // volta pro login
        }
      });
      // make console slower/annoying
      const el = new Image();
      Object.defineProperty(el, "id", { get() { throw new Error("DevTools!"); }});
      setInterval(() => console.log(el), 1000);
    })();

    // 2) Client-side session check (UX)
    async function checkSessionClient() {
      try {
        // first check sessionStorage (if your login stored token there)
        const token = sessionStorage.getItem("sessionId");
        if (!token) {
          // sem token no sessionStorage -> tenta apenas redirect (middleware server fará validação real)
          window.location.replace("/");
          return;
        }

        // optional: double-check with the verify endpoint (improves UX: show spinner then load)
        ...
        if (res.ok) {
          ...
          if (data.ok) {
            // tudo certo, permite mostrar a página
            return;
          }
        }
      } catch (err) {
        // se falhar verificação (ex: network) redireciona pro login
      }
      window.location.replace("/");
    }

    // roda check rápido no carregamento
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", checkSessionClient);
    } else checkSessionClient();
  })();
// ===================== CONFIG FIREBASE ===========================
