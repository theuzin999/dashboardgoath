...
  /** Checa se as três últimas velas foram positivas. (CORRIGIDA) */
  function hasThreeConsecutivePositives(colors){
      const L = colors.length;
      if(L < 3) return false;
      return colors[L-1] !== "blue" && colors[L-2] !== "blue" && colors[L-3] !== "blue";
  }
...
    }
    
    // Verifica a condição de LIBERAÇÃO (Três positivas consecutivas - CORRIGIDO)
    if (pendingTwoBlueBlock && hasThreeConsecutivePositives(colors)) {
        // Se a condição for atendida, libera o bloqueio
        pendingTwoBlueBlock = false;
        safeFeed("info", "Bloqueio 2+ Azuis OFF: Três positivas consecutivas confirmadas.");
    }
    // =========================================================================
