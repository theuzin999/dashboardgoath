...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /** Checa se há dois azuis (B-B) imediatamente no final da sequência. */

  function hasImmediateTwoBlues(colors) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function hasForce(colors, N=6){ 
    ...
    if(L < N) return false;
    for(let i=L-N; i<L-2; i++){ 
       if(isPos(colors[i]) && isPos(colors[i+1]) && isPos(colors[i+2])){ return true; }
    }
    return false;
  }
  ...
  function onNewCandle(arr){
    ...
    // BLOQUEIO 2+ AZUIS ATRÁS DA ÚLTIMA P
    const bluesBeforeLastP = bluesBeforeLastPositive(colors);
    if (bluesBeforeLastP >= 2) {
        if (!pendingTwoBlueBlock) {
            pendingTwoBlueBlock = true;
    ...
    console.log(`[CANDLE ${last.idx}] CorrGate: ${corrGate} | Pred: ${(predN*100).toFixed(0)}% | BlueRun: ${finalBlueRunNow(colors)} | 2+Block: ${pendingTwoBlueBlock}`);
    ...
    if(pending?.stage === 'G1_WAIT' || pending?.stage === 'G2_WAIT'){
      ...
      // BLOQUEIO ABSOLUTO: 2 AZUIS SEGUIDOS (B-B)
      if (hasImmediateTwoBlues(colors)) {
        safeFeed("warn", `${stage} BLOQUEADO: 2 azuis seguidos (B-B) no final`);
        setCardState({
          active: false,
          awaiting: true,
          title: `Aguardando ${stage}`,
          sub: "2 azuis seguidos (B-B) — entrada PROIBIDA"
        });
        return;
      }

      // FORÇA (só ativa se NÃO tiver B-B)
      const forceDetected = hasForce(colors, 6);
      if (forceDetected) {
        pendingTwoBlueBlock = false;
        safeFeed("info", `${stage} liberado por FORÇA (3P em 6 velas)`);
      } 
      // BLOQUEIO 2+ AZUIS ATRÁS DA ÚLTIMA P
      else if (pendingTwoBlueBlock) {
        safeFeed("warn", `${stage} pausado — 2+ azuis antes da última P`);
        setCardState({active:false, awaiting:true, title:`Aguardando ${stage}`, sub:`Bloqueio 2+ azuis ativo`});
        return;
      }

      // XADREZ
      if (isXadrezAlternado4(colors) && colors[colors.length-1] !== "blue") {
        safeFeed("warn", `${stage} pausado (xadrez) — aguarde azul`);
        setCardState({active:false, awaiting:true, title:`Aguardando ${stage}`, sub:"Xadrez: aguarde B"});
        return;
      }

      // MESMA ESTRATÉGIA DO G0
      const currentStrategy = analysis?.name || (window.seguidinhaOn ? "seguidinha" : (forceDetected ? "forca" : null));
      if (currentStrategy && currentStrategy === lastG0Strategy) {
        safeFeed("warn", `${stage} pausado — mesma estratégia do G0`);
        setCardState({active:false, awaiting:true, title:`Aguardando ${stage}`, sub:"Estratégia repetida"});
        return;
      }
      ...
      pending.strategy = currentStrategy || "correção";
      ...
      gateTag.textContent = "Gatilho: " + (analysis?.gate || (forceDetected ? "Força 3P/6" : "seguidinha"));
      safeFeed("warn", `SINAL 2x (${stage}) — entrar APÓS ${pending.afterMult}`);
      return;
    }

    // BLOQUEIO 3+ AZUIS (NOVO G0)
    if (currentBlueRun >= 3) {
